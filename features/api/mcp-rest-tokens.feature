...
Feature: C-mcp REST Tokens
  ...
  Scenario: PUT token override without version in body returns 400 MISSING_VERSION
    ...
    When I PUT "/api/projects/occ-proj/tokens/color.primary" with bearer token "test-secret-123" and body:
      """
      {"value": "#ff0000"}
      """
  ...
  Scenario: PUT token override with non-number version returns 400 INVALID_VERSION
    ...
    When I PUT "/api/projects/occ-proj2/tokens/color.primary" with bearer token "test-secret-123" and body:
      """
  ...
  Scenario: PUT token override with stale version returns 409 CONFLICT without currentVersion
    ...
    When I PUT "/api/projects/occ-proj3/tokens/color.primary" with bearer token "test-secret-123" and body:
      """
